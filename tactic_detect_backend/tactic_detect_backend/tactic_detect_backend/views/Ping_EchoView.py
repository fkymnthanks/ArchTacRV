...
pingecho = Blueprint("pingecho", __name__)


@pingecho.route('/restoremethod', methods=['POST'])
def restore_method():
    try:
        ...
        for method in data:
            ...
            item = models.Ping_Echo.Ping_echo(user_email, dataset_no, method_type, method_name)
            db.session.add(item)
            db.session.commit()
...
def get_dataset_no():
    sql = 'SELECT MAX(dataset_no) FROM `ping_echo`'
